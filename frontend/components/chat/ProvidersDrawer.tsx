...
import { Star, MapPin, Clock, Zap, Tag, Phone, Loader2, Mail, Globe, MessageCircle, ExternalLink, User, Navigation, ChevronRight } from 'lucide-react'
...
export function ProvidersDrawer({
  ...
  useEffect(() => {
    if (providers.some(p => p.location_lat != null && p.location_lng != null)) {
      if (userLat === null && userLng === null && !geoLoading && !geoError) {
        requestLocation()
      }
    }
  }, [providers, userLat, userLng, geoLoading, geoError, requestLocation])
  ...
  return (
    <aside className="hidden xl:flex xl:flex-col xl:w-[440px] flex-shrink-0 border-l border-[rgba(255,255,255,0.08)] bg-[#101522]/80 h-screen overflow-y-auto scrollbar-thin relative z-10">
      ...
      {/* Sort filters */}
      <div className="flex gap-2 px-4 py-3 border-b border-[rgba(255,255,255,0.08)]">
                ...
                <div className="flex items-start gap-3">
                  {provider.avatar_url ? (
                    <img
                      ...
                      className="w-12 h-12 rounded-[14px] border border-[rgba(255,255,255,0.08)] object-cover flex-shrink-0"
                    />
                  ) : (
                    <div className="w-12 h-12 rounded-[14px] bg-[#6E42FF]/10 flex items-center justify-center text-sm font-bold text-[#6E42FF] flex-shrink-0">
