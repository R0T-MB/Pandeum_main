...
import { Star, MapPin, Clock, Zap, Navigation, Car, Footprints, Bike, MessageCircle, Phone, Heart, ChevronRight, ShieldCheck } from 'lucide-react'
...
export function RightPanel({ providers = [] }: RightPanelProps) {
    ...
    return (
      <aside className="hidden xl:flex xl:flex-col xl:w-[340px] flex-shrink-0 border-l border-[rgba(255,255,255,0.06)] bg-[#080B14] h-screen overflow-y-auto scrollbar-thin">
        <div className="p-4 space-y-4">
          <div className="bg-[#111827] rounded-xl border border-[rgba(255,255,255,0.06)] p-8 text-center">
            <div className="w-12 h-12 rounded-xl bg-[#7C3AED]/10 flex items-center justify-center mx-auto mb-4">
              <Zap size={20} className="text-[#7C3AED]" strokeWidth={1.5} />
            </div>
            <h3 className="text-sm font-semibold text-white mb-2">Tus recomendaciones aparecerán aquí</h3>
  ...
  return (
    <aside className="hidden xl:flex xl:flex-col xl:w-[340px] flex-shrink-0 border-l border-[rgba(255,255,255,0.06)] bg-[#080B14] h-screen overflow-y-auto scrollbar-thin">
      <div className="p-4 space-y-4">
        <div className="bg-[#111827] rounded-xl border border-[rgba(255,255,255,0.06)] p-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-semibold text-white">Te puede interesar</h3>
            <button className="text-[11px] font-medium text-[#7C3AED] hover:text-white transition-colors">
              Ver más
            </button>
          </div>
          <div className="space-y-2.5">
            {providers.map((p) => {
              const distance = p.distance_km != null ? `${p.distance_km.toFixed(1)} km` : null
              return (
                <div key={p.provider_id} className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-[rgba(255,255,255,0.03)] transition-all duration-200 group">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      ...
                      {p.available_now && <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 flex-shrink-0" />}
                    </div>
        ...
        {featured && (
          <div className="bg-[#111827] rounded-xl border border-[rgba(255,255,255,0.06)] p-4 space-y-3">
            ...
            <div className="flex items-start gap-3">
              <div className="w-14 h-14 rounded-xl bg-[#7C3AED]/10 flex items-center justify-center text-lg font-bold text-[#7C3AED] flex-shrink-0">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="bg-[rgba(255,255,255,0.03)] rounded-lg px-3 py-2">
                <p className="text-[10px] text-[#9CA3AF]">Costo estimado</p>
              ...
              <div className="bg-[rgba(255,255,255,0.03)] rounded-lg px-3 py-2">
                ...
                <p className="text-xs font-medium text-white">
                  {featured.distance_km != null ? `${featured.distance_km.toFixed(1)} km` : '—'}
                </p>
              </div>
              <div className="bg-[rgba(255,255,255,0.03)] rounded-lg px-3 py-2">
            ...
            <div className="flex items-center gap-2 pt-1">
              <button className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-[11px] font-medium bg-[#7C3AED] hover:bg-[#6D5EF8] text-white transition-all duration-200">
        ...
        <div className="bg-gradient-to-br from-[#0E1422] to-[#080B14] rounded-xl border border-[rgba(255,255,255,0.06)] p-4 pandeum-glow">
          <h3 className="text-sm font-semibold text-white mb-3">Ruta sugerida</h3>
          <div className="text-center py-3">
            ...
            <p className="text-xs text-[#9CA3AF] mt-1">
              {featured?.distance_km != null
                ? `${featured.distance_km.toFixed(1)} km · Llegada estimada ${new Date(Date.now() + Math.round(featured.distance_km / 0.1) * 60000).toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit' })}`
